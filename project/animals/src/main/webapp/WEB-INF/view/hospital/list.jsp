...
<div>
	<main class="main">
		...
		<h2 class="main title">동물병원</h2>

		<div class="search-form">
			<h3 class="hidden">동물병원 검색폼</h3>
			<form class="table-form">
				<fieldset>
					<legend class="hidden">동물병원 검색분류</legend>

					<label></label> <select name="f">
						<option value="name">이름</option>
						<option value="address">주소</option>
						<option value="tel">지역번호</option>
			...
			<table border="1" class="table">
				...
				<tbody>
					<c:forEach var="h" items="${list}">
						<tr>
							<td style="text-align: center; width: 50px;">${h.id}</td>
							<td style="text-align: center; width: 250px"
								class="title ident text-align-left"><a
								href="detail?id=${h.id}">${h.name}</a></td>
							<td style="text-align: center; width: 625px;">${h.address}</td>
						</tr>
					</c:forEach>
				</tbody>
			</table>
